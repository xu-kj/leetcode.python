# 5710. Number of Orders in the Backlog
...
Return the total **amount** of orders in the backlog after placing all the orders from the input. Since this number can be large, return it **modulo** `1e9 + 7`.
...
* `1 <= orders.length <= 1e5`
* `orders[i].length == 3`
* `1 <= price_i, amount_i <= 1e9`
* `orderType_i` is either `0` or `1`.

[problem]: https://leetcode.com/contest/weekly-contest-233/problems/number-of-orders-in-the-backlog/
